...
gaia_g_phot['g_unc'] = np.abs(-2.5/np.log(10)*gaia_data['phot_g_mean_flux_error']/gaia_data['phot_g_mean_flux'])
gaia_rp_phot['rp_unc'] = np.abs(-2.5/np.log(10)*gaia_data['phot_rp_mean_flux_error']/gaia_data['phot_rp_mean_flux'])

# TODO: drop nans using astropy Tables method.


# TODO: ingest Gaia photometry
def add_gaia_photometry(db):

    ingest_photometry(db, gaia_rp_phot['db_names'], 'GAIA2.Grp', gaia_rp_phot['phot_rp_mean_mag'], gaia_rp_phot['rp_unc'],
                      'GaiaDR2', ucds='em.opt.R', telescope='Gaia', instrument='Gaia', verbose=True)

    ingest_photometry(db, gaia_g_phot['db_names'], 'GAIA2.G', gaia_g_phot['phot_g_mean_mag'], gaia_g_phot['g_unc'],
                      'GaiaDR2', ucds='em.opt', telescope='Gaia', instrument='Gaia', verbose=True)
