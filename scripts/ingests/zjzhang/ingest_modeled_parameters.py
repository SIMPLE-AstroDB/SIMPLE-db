...
T7Y1_link = (
    "scripts/ingests/zjzhang/T7_to_Y1_benchmarks08062025.csv"
)
...
skipped1, reason1, skipped2, reason2 = [], [], [], []
...
for row in L6T6_table:
    ...
    if(len(source_name)>1):
        skipped2.append(row["object"])
        reason2.append("more than one match")
        continue
    param_counter = 0
    while(param_counter < 4):
        ...
        try:
            ...
            skipped1.append(row["object"])
            reason1.append("no source found, L6T6")
...
for row in T7Y1_table:
    ...
    while(param_counter < 5):
        ...
        try:
                    ...
                    db.ModeledParameters.insert().values(
                        {
                            ...
                            "lower_error": lower_error,
                            "unit": unit,
                            "comments": "Ingested from compilation by Zhang et al. (2021ApJ...911....7Z)",
                            "reference": ref
                        }
                    )
                )
            ...
            skipped1.append(row["object"])
            reason1.append("no source found, T7Y1")

logger.info("done")

skipped1_table = Table([skipped1, reason1], names=["Skipped", "Reason"])
skipped1_table.write(
    "scripts/ingests/zjzhang/zj_modeled_parameters_no_source.csv",
    delimiter=",",
    overwrite=True,
    format="ascii.ecsv",
)

skipped2_table = Table([skipped2, reason2], names=["Skipped", "Reason"])
skipped2_table.write(
    "scripts/ingests/zjzhang/zj_modeled_parameters_more_source.csv",
    delimiter=",",
    overwrite=True,
    format="ascii.ecsv",
)
