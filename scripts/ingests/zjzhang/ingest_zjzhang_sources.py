...
for source in zhang_sheet_table:
    zhang_name = source["NAME"].replace("\u2212", "-")
    zhang_name = zhang_name.replace("\u2212", "-")
    zhang_name = zhang_name.replace("\u2013", "-")
    zhang_name = zhang_name.replace("\u2014", "-")
    match = None
    ...
    if len(match) == 0:
        ...
        try:
            ...
            ingest_source(
                ...
                raise_error=True,
                search_db=True,
                comment="Discovery reference from the zjzhang archive",
            )  # ingest new sources
            ingested += 1
        ...
        try:
            ingest_name(
                db, source=match[0], other_name=zhang_name, raise_error=True
